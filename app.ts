...
let boardruns:(string|number)[]=["DOT BAll",1,2,3,4,5,6,"OUT"]
let players_data:number[][]=[[ 5, 30, 25, 10, 15, 1, 9, 5],[10, 40, 20, 5 , 10, 1, 4, 10] ,[20, 30, 15,5 , 5, 1, 4, 20],[ 30, 25, 5, 0, 5, 1, 4, 30]]
let player_strike:number[],finalresult:number,evaluation:number;
let scoredata,scoring:number,overs:number,count:number,player:number=0,outplayers:string[]=[];
let players_score:number[]=[0,0,0,0];
let players_balls:number[]=[0,0,0,0]
let finalscore:(string|number)[][]=[],notoufinalscore:(string|number)[][]=[];
let players:number[]=[0,0,0,0];
let players_name=["Kirat Boli","N.S.Nodhi","R Rumrah","Shashi Henra"]
let names=["Kirat Boli","N.S.Nodhi","R Rumrah","Shashi Henra"]
let runs:number=40,finaloutput:number=0;
let traverse:number;
let ball_left:number=24;
...
function strike(overs:number,count:number,player:number){
    ...
    if(player===0)
        player_strike=players_data[0] 
    else if(player===1)
         player_strike=players_data[1] 
    else if(player===2)
        player_strike=players_data[2]   
    else if( player===3)
      player_strike=players_data[3] 
...
{ 
    ...
    return 
        ...
        {
            // let name:string=players_name[player]
            // let score:number=players_score[player]
            // let balls:number=players_balls[player]
        ...
        if(scoring==2 || scoring ||scoring==6)
        if(player==0) player=1
        else player=0
        strike(overs,count,player)
    }
